...
class Correlation:
    ...
    def multilayer_correlation(cls, query_feats, support_feats, stack_ids):
        ...
        for idx, (query_feat, support_feat) in enumerate(zip(query_feats, support_feats)):
            ...
            support_feat_corr = support_feat.view(bsz, ch, -1)
            support_feat_corr = support_feat_corr / (support_feat_corr.norm(dim=1, p=2, keepdim=True) + eps)

            bsz, ch, ha, wa = query_feat.size()
            query_feat_corr = query_feat.view(bsz, ch, -1)
            query_feat_corr = query_feat_corr / (query_feat_corr.norm(dim=1, p=2, keepdim=True) + eps)
            corr = torch.bmm(query_feat_corr.transpose(1, 2), support_feat_corr).view(bsz, ha, wa, hb, wb)
